...
| SEC-016 | Security headers | High | Plan and preview config drafts exist; host config missing | Missing headers increase XSS/clickjacking/content sniffing risk | Configure CSP, HSTS, frame, referrer, and content-type headers on selected host | Yes for public beta if hosting allows | Yes |
